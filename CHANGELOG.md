...
### Bug Fixes
- **Multibranch `push` empty-args crash** — Fixed a Bash 3.2 `set -u` crash in `buildgit push` when running against multibranch jobs with no explicit push args (for example, `buildgit push`). Branch inference now safely falls back to the current git branch.
