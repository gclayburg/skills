...
- Set to `true` if the spec should be broken into a multi-chunk implementation plan before implementation. Set to `false` if it can be implemented in a single pass.
...
- [ ] **Update `jbuildmon/skill/buildgit/references/reference.md`** if the output format or available options change (new flags, new output sections, changed progress bar format, etc.).
...
- [ ] **Update `CLAUDE.md` AND `README.MD`** (at the repository root): if the output of `buildgit --help` changes in any way (new or changed options, wording, etc)
...
- plan files are created in `specs/`, not `specs/todo/` — they are ready-to-implement artifacts
- see `taskcreator.md` for details on how plans and chunks are created and implemented

### Multi-chunk plan workflow tiers

When implementing a spec via a multi-chunk plan (`implement-spec.sh --ralph-loop`), the
Implementation Workflow is split into two tiers:
...
- Update CHANGELOG.md, README.md, SKILL.md, reference.md
- Update spec state to IMPLEMENTED
- Move reference files to `specs/done-reports/`
- Update CLAUDE.md if buildgit --help output changed
- Push and verify CI
